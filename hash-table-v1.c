...
{
	int error = pthread_mutex_lock(&mutex); //lock critical section
	struct hash_table_entry *hash_table_entry = get_hash_table_entry(hash_table, key);
	struct list_head *list_head = &hash_table_entry->list_head;
	
	if(error !=0) 
	{
		exit(error); //exit with error code 
	}
	struct list_entry *list_entry = get_list_entry(hash_table, key, list_head);
	...
	if(error !=0) 
	{
		exit(error); //exit with error code; check if unlock works
	}
	...
	list_entry->key = key;
	list_entry->value = value;
	error = pthread_mutex_lock(&mutex); //lock critical section
	if(error !=0) 
	{
		exit(error); //exit with error code 
	}

...
struct hash_table_entry {
	...
	pthread_mutex_t mutex; //declare a lock per hash table entry
};
...
struct hash_table_v2 *hash_table_v2_create()
{
	...
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		//initalize mutex for every entry in hash table
		int error = pthread_mutex_init(&entry->mutex, NULL);
		if (error != 0)	//if not success code of 0, exit w error code
		{
			exit(error);	
		}
	}
...
void hash_table_v2_add_entry(struct hash_table_v2 *hash_table, const char *key, uint32_t value)
{
	...
	struct hash_table_entry *hash_table_entry = get_hash_table_entry(hash_table, key);
	//lock critical section to safely add entry
	//with error checking if failed.
...
void hash_table_v2_destroy(struct hash_table_v2 *hash_table)
{
	for (size_t i = 0; i < HASH_TABLE_CAPACITY; ++i) {
		...
		//destroy every lock per entry w error checking
